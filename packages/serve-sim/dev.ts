 ...
 *
 * The whole host surface — the control socket (`/exec-ws`), the network
 * inspector, devtools, grid, and every SSE side-channel — is served by the
 * production `simMiddleware`, mounted on a Node `http` server exactly like
 * `servePreview` does. This file only layers on the dev-only concerns:
 ...
 *   • `/grid/api/start` re-pointed at the local source instead of the published
 *     `serve-sim` the middleware would otherwise resolve.
 */
import { readdirSync, readFileSync, existsSync, unlinkSync, mkdirSync, writeFileSync, watch } from "fs";
...
import { STATE_DIR, type ServeSimState, readServeSimStates, selectServeSimState } from "./src/state";
import { simMiddleware, previewConfigForState } from "./src/middleware";
...
// The preview client routes every host request — shell execs, simulator
// settings, and the SSE side-channels — through one `/exec-ws` control socket,
// authenticated with this per-process token (injected into the page config).
const EXEC_TOKEN = randomBytes(32).toString("base64url");

// The same connect-style middleware production runs. We point its advertised
// CLI binary at our local source so the sidebar's `serve-sim …` calls (network,
// camera, ui) execute from this checkout instead of a globally-installed copy.
const middleware = simMiddleware({ basePath: "/", execToken: EXEC_TOKEN, serveSimBin: SERVE_SIM_BIN });

// The dev server serves at the root, so the base path is "" (same as
// production's `simMiddleware({ basePath: "/" })`).
...
function buildHtml(selectedDevice?: string | null): string {
  ...
  // basePath + exec token.
  const config = state ? devPreviewConfig(state) : { basePath: "", execToken: EXEC_TOKEN };
...
const server = createServer((req, res) => {
  ...
  // Dev-only routes intercept first; everything else is the production surface.
...
// The exec WebSocket control channel rides the upgrade handshake, same as
// `servePreview`. Works under Bun via the `ws` module's native substitution.
server.on("upgrade", (req, socket, head) => {
  if (!middleware.handleUpgrade?.(req, socket, head)) socket.destroy();
});

// MJPEG streams + SSE channels are long-lived; clear the default socket
// timeouts so they aren't torn down mid-stream.
...
// Publish a discovery file so `serve-sim network` (run from the sidebar or a
// shell) finds this dev server's origin + token, exactly like the production
// `serve-sim` server does. Cleaned up on exit.
...
try {
  ...
  process.on("exit", () => {
    // Only remove the discovery file if it's still ours — a production
    // `serve-sim` server may have replaced it while we were running.
    try {
      const cur = JSON.parse(readFileSync(previewStateFile, "utf8"));
      if (cur.pid === process.pid) unlinkSync(previewStateFile);
    } catch {}
  });
